...
export default function CartCard({ id, stateOfCart }) {
  ...
  function addFunc() {
    const sync = stateOfCart[0].findIndex((obj) => {
      if (obj === undefined) {
        return false
      }
      return obj.id === id
    })
    if (sync === -1) stateOfCart[1](stateOfCart[0].concat({ id: id, count: 1 }))
    else {
      const prev = JSON.parse(JSON.stringify(stateOfCart[0]))
      const newPrev = prev.map((obj) => {
        if (obj.id === id) {
          obj.count++
        }
      })
  ...
  return (
    <div className='card' key={data ? data.id : null}>
      <div className='imgContainer'>
        <div className='addBtn' onClick={addFunc}>
      ...
      <div className='infoContainer'>
        <div className='priceContainer'>
          <div className='salePrice'>
            ...
            <div className='price'>
              <h2>{data ? spaces(realPrice(data.price)) : null}</h2>
            </div>
          </div>
